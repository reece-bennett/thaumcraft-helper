...
public class Graph {
  ...
  /*
   * public ArrayList<Node> getPath(String start, String end) { // Start by resetting the parents of all nodes in the
   * graph for (Node n : nodes.values()) { n.setParent(null); }
   * 
   * LinkedList<Node> frontier = new LinkedList<>(); LinkedList<Node> explored = new LinkedList<>();
   * 
   * // Start the search by adding the start node to the frontier frontier.add(nodes.get(start));
   * 
   * while (!frontier.isEmpty()) { Node current = frontier.removeFirst();
   * 
   * if (current.getName().equals(end)) { // If the current node is the end point the search is complete, return the
   * path ArrayList<Node> path = new ArrayList<>(); Node parent = current; while (!parent.getName().equals(start)) {
   * path.add(0, parent); parent = parent.getParent(); } path.add(0, parent); return path;
   * 
   * } else if (!explored.contains(current)) { // Else expand all children of the current node and if not already in
   * frontier or explored, // add them to the frontier explored.add(current); for (Node n : current.getNeighbours()) {
   * if (!frontier.contains(n) && !explored.contains(n)) { n.setParent(current); frontier.add(n); } } } } return null; }
   */
